...
document.getElementById('loginJugadorForm')?.addEventListener('submit', function(e){
  ...
  fetch('data.json').then(r=>r.json()).then(db=>{
    ...
    let uexist = db.players.find(p=> p.username.toLowerCase()===user.toLowerCase());
    if(uexist){ document.getElementById('loginJugadorError').innerText='❌ Contraseña incorrecta'; return; }
