// auth.js - supports admin/admin1234, MIGI/MIGUI23 and players in data.json
...
async function loadData(){
  ...
  try{
    ...
    APP_DATA = { meta:{name:'Quiniela 32'}, players:[] };
  }
  return APP_DATA;
}
document.addEventListener('DOMContentLoaded', async ()=>{
  ...
  loginBtn.addEventListener('click', ()=>{
    ...
    if(!user || !pass){ msg.textContent='Introduce usuario y contraseña'; return; }
    ...
    const player = data.players.find(p => p.name === user && p.password === pass);
    if(player){
      sessionStorage.setItem('session', user);
      window.location.href = 'jugador.html?user=' + encodeURIComponent(user);
      return;
